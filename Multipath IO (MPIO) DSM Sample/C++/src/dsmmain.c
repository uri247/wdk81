...
--*/
{
    ...
    switch (groupEntry->LoadBalanceType) {
        ...
        case DSM_LB_LEAST_BLOCKS: {
            ...
            //
            // 1. The cache is disabled or
            // 2. This is not a read/write request or
            // 3. This is a read/write request
            ...
            if ((isRead || isWrite) && (groupEntry->UseCacheForLeastBlocks)) {
                ...
                //
                if ((startLba >= lastPathUsed->LastLba) &&
                    ((isRead) ||
                                            ...
                                            if (InterlockedCompareExchange((LONG volatile*)&group->Throttled, 0, 1)) {
                                                ...
                                                if (!NT_SUCCESS(resumeStatus)) {
                                                    ...
                                                    TracePrint((TRACE_LEVEL_WARNING,
                                                                ...
                                                                deviceInfo));
                                                    //
                                                    // As per MPIO code, there are many error conditions in which MPDEV state will change with this call
                                                    // but status returned will not be success. So that means all subsequent call to resume will also fail
                                                    // At present we should not hit this code path and hence assert is there. And we will ignore this failure
                                                    //
                                                }
...
--*/
{
    ...
    if (InterlockedCompareExchange((LONG volatile*)&group->Throttled, 0, 1)) {
        ...
        if (!NT_SUCCESS(resumeStatus)) {
            ...
            TracePrint((TRACE_LEVEL_WARNING,
                        TRACE_FLAG_RW,
                        "DsmpPhase2ProcessPathFailingALUA (DevInfo %p): Resume IO failed.\n",
                        deviceInfo));
            //
            // As per MPIO code, there are many error conditions in which MPDEV state will change with this call
            // but status returned will not be success. So that means all subsequent call to resume will also fail
        ...
        if (NT_SUCCESS(status) || status == STATUS_BUFFER_OVERFLOW) {
            ...
            if (serviceAction == RESERVATION_ACTION_READ_KEYS) {
                ...
                for (i = 0; i < numberOfKeys; i++) {
                    ...
                    REVERSE_BYTES_QUAD(&prKey, &(prInRegistrationList->ReservationKeyList[i]));
                    TracePrint((TRACE_LEVEL_INFORMATION,
                                TRACE_FLAG_IOCTL,
