...
--*/
{
    ...
    if (PropertyRequest->InstanceSize >= sizeof(ULONG))
    {
        ...
        if (IsRenderDevice())
        {
            if (GetSystemPinId() == nPinId)
            {
                ntStatus = STATUS_SUCCESS;
            }
            else if (GetSourcePinId() == nPinId ||
                     GetLoopbackPinId() == nPinId ||
                     (IsOffloadSupported() && GetOffloadPinId() == nPinId))
            {
        ...
        else // capture device.
        {
            switch(nPinId)
            {
                case KSPIN_WAVEIN_HOST:
                    ntStatus = STATUS_SUCCESS;
                    break;

                case KSPIN_WAVE_BRIDGE:
                    ntStatus = STATUS_NOT_SUPPORTED;
                    break;
            }
        }
    ...
    else if (PropertyRequest->Verb & KSPROPERTY_TYPE_GET)
    {
        ...
        if (IsRenderDevice() || !bHfpNrecPresent)
        {
