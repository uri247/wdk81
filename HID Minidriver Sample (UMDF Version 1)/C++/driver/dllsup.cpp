...
Abstract:
    ...
    This module contains the implementation of the Driver's entry point and 
    its exported functions for providing COM support.
    ...
    These are defined in internal.h for the sample. If you choose to use a 
    different primary include file, you should ensure they are defined there as 
    well.
...
/*++
  ...
  Routine Description:
    ...
    This routine is called by COM in order to instantiate the driver callback 
    object and do an initial query interface on it.
...
--*/
{
    ...
    *Interface = NULL;
    ...
    if (IsEqualCLSID(ClassId, CLSID_MyDriverCoClass) == false)
    {
        Trace(
            TRACE_LEVEL_ERROR,
            "ERROR: Called to create instance of unrecognized class (%!GUID!)",
            &ClassId
            );
