                                          ...
                                          _In_ const UINT32 stringCount)
{
      ...
      pFilter->layerKey == FWPM_LAYER_STREAM_V6)
   {
      status = RPCInvokeScenarioFastStreamInjection(wfpSamplerBindingHandle,
                                                    ...
                                                    pFilter);
      if(status != NO_ERROR)
         HlprLogError(L"FastStreamInjectionScenarioExecute : RpcInvokeScenarioBasicPacketInjection() [status: %#x]",
                      status);
      else
         HlprLogInfo(L"FastStreamInjectionScenarioExecute : RpcInvokeScenarioBasicPacketInjection() [status: %#x]",
                     status);
   }
   else
   {
