...
#pragma warning(pop)
   ...
   *ppInjectionData = 0;
   ...
   if(status != STATUS_SUCCESS)
   {
      DbgPrintEx(DPFLTR_IHVNETWORK_ID,
                 DPFLTR_ERROR_LEVEL,
                 " !!!! PerformBasicPacketInjectionForward: FwpsAllocateCloneNetBufferList() [status: %#x]\n",
                 status);
   ...
   if(status != STATUS_SUCCESS)
   {
      DbgPrintEx(DPFLTR_IHVNETWORK_ID,
                 DPFLTR_ERROR_LEVEL,
                 " !!!! PerformBasicPacketInjectionForward: FwpsInjectForwardAsync() [status: %#x]\n",
                 status);
...
_IRQL_requires_max_(DISPATCH_LEVEL)
_IRQL_requires_same_
VOID ClassifyBasicPacketInjection(_In_ const FWPS_INCOMING_VALUES* pClassifyValues,
                                  _In_ const FWPS_INCOMING_METADATA_VALUES* pMetadata,
                                  _Inout_opt_ VOID* pNetBufferList,
                                  _In_opt_ const VOID* pClassifyContext,
                                  _In_ const FWPS_FILTER* pFilter,
