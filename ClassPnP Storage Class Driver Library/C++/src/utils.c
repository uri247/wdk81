...
--*/
{
    ...
    switch (ioLogMessageContextHeader->ErrorCode) {
        ...
        case IO_WARNING_IO_OPERATION_RETRIED: {
            ...
            //
            // We need to allocate enough space for 3 insertion strings:
            // 1. A ULONGLONG representing the LBA which means a max of 20 digits,
            // plus one for the NULL character.
    ...
    logEntrySize = sizeof(IO_ERROR_LOG_PACKET) + sizeof(ULONG) + senseBufferSize;

    //
    // Make sure the string offset is WCHAR-aligned (the insertion strings
    // come after the sense buffer in the dump data, if any).
    //
    stringOffset = ALIGN_UP_BY(logEntrySize, sizeof(WCHAR));
    ...
    // insertion strings.
    //
    logEntrySize = stringOffset + stringsBufferLength;
