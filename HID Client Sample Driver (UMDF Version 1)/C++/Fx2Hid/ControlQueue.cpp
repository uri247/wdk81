...
--*/
{
	...
	switch (ControlCode)
	{
		...
		case IOCTL_OSRUSBFX2_READ_SWITCHES: 
		{
			...
			//
			// Get the data buffer and use it to get the switch state from the
			// device.
			//
		...
		case IOCTL_OSRUSBFX2_GET_INTERRUPT_MESSAGE: 
		{
			...
			else
			{
				...
				hr = FxRequest->ForwardToIoQueue(m_Device->GetSwitchChangeQueue());
		...
		{
			//
			// These IOCTLs are not supported from the HID collection for the FX2 device.
			//
